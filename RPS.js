...
const beat=document.getElementById('beat'); const you=document.getElementById('you'); const bot=document.getElementById('bot');
const setuser=document.querySelector("#user-score");
const setcomp=document.querySelector("#comp-score");

const generate=() =>{
    const options= ["rock","paper","scissor"];
    const i= Math.floor(Math.random()*3); return options[i];
}
...
function winner(user,userchoice,compchoice){
    if(user){
        ...
        message.style.backgroundColor ="rgb(29, 189, 136)"; beat.innerText=`${userchoice} beats ${compchoice}`;
    }
    else{
        compscore++;
        setcomp.innerText= compscore; console.log("lose");
        msg.innerText= "You lose";
        message.style.backgroundColor ="#6e0d25"; beat.innerText=`${compchoice} beats ${userchoice}`;
    }
    if(userscore>compscore){
        bot.style.boxShadow= "10px 7px 20px black inset"; you.style.boxShadow= "10px 7px 20px black";}
    else if(userscore===compscore){
        bot.style.boxShadow= "10px 7px 20px black"; you.style.boxShadow= "10px 7px 20px black";
    }
    else{
